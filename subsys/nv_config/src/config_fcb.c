...
int conf_fcb_src(struct conf_fcb *cf)
{
	...
	while (1) {
		rc = fcb_init(CONFIG_NV_CONFIG_FCB_FLASH_AREA, &cf->cf_fcb);
		if (rc) {
			return -EINVAL;
		}
		...
		if (fcb_free_sector_cnt(&cf->cf_fcb) < 1) {
			//rc = flash_area_open(cf->cf_fcb.f_area_id, &fap);
			//if (rc) {
			//	return -EINVAL;
			//}
...
static void conf_fcb_compress(struct conf_fcb *cf)
{
	...
	//rc = flash_area_open(cf->cf_fcb.f_area_id, &loc1.fap);
	//if (rc) {
	//	return; /* XXX */
	//}
	...
	while (fcb_getnext(&cf->cf_fcb, &loc1.loc) == 0) {
		 ...
		 */
		rc = flash_area_read(loc1.fap, FCB_ENTRY_FA_DATA_OFF(loc1.loc), buf1,
		...
		rc = flash_area_write(loc2.fap, FCB_ENTRY_FA_DATA_OFF(loc2.loc), buf1,
		  loc1.loc.fe_data_len);
		if (rc) {
			continue;
		}
