...
#ifdef CONFIG_NV_CONFIG_FS
#include <fs.h>

static struct conf_file config_init_conf_file = {
	.cf_name = CONFIG_NV_CONFIG_NFFS_FILE,
	.cf_maxlines = CONFIG_NV_CONFIG_NFFS_MAX_LINES
};
...
#elif defined(CONFIG_NV_CONFIG_FCB)
#include "fcb.h"
#include "config/config_fcb.h"

static struct flash_sector conf_fcb_area[CONFIG_NV_CONFIG_FCB_NUM_AREAS + 1];

static struct conf_fcb config_init_conf_fcb = {
	.cf_fcb.f_magic = CONFIG_NV_CONFIG_FCB_MAGIC,
...
static void config_init_fcb(void)
{
	...
	rc = flash_area_get_sectors(CONFIG_NV_CONFIG_FCB_FLASH_AREA, &cnt,
								conf_fcb_area);
	if (rc != 0) {
		k_panic();
	}
	...
	rc = flash_area_open(CONFIG_NV_CONFIG_FCB_FLASH_AREA, &fap);
...
config_pkg_init_stage2(void)
{
	 ...
	 */
#ifdef CONFIG_NV_CONFIG_FS
	fs_mkdir(CONFIG_NV_CONFIG_NFFS_DIR);
#endif
}
