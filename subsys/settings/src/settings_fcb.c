...
static void settings_fcb_compress(struct settings_fcb *cf)
{
	...
	while (fcb_getnext(&cf->cf_fcb, &loc1.loc) == 0) {
		...
		size_t len_read;
		rc = settings_line_raw_read(val1_off + 1, name2, 1, &len_read,
					    &loc1);

		if (rc || len_read == 0 || name2[0] == '\0') {
		...
		while (fcb_getnext(&cf->cf_fcb, &loc2.loc) == 0) {
			...
			if ((val1_off == val2_off) && !memcmp(name1, name2, val1_off)) {
				copy = 0;
				break;
			}
		}
...
static int write_handler(void *ctx, off_t off, char const *buf, size_t len)
{
	...
	return flash_area_write(entry_ctx->fap, FCB_ENTRY_FA_DATA_OFF(entry_ctx->loc) + off, buf, len);
}
