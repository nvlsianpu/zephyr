...
int settings_line_write(const char *name, const char *value, size_t val_len, off_t w_loc,
			void *cb_arg)
{
 ...
 */
int settings_line_raw_read_until(off_t seek, char *out, size_t len_req, size_t *len_read, char const *until_char,
		       void *cb_arg)
{
	size_t rem_size, len;
			   ...
			   size_t *len_read, void *cb_arg)
{
	...
	while (rem_size) {
		...
		rc = settings_line_raw_read(val_off + seek_begin, enc_buf, read_size,
					    &read_size, cb_arg);
		if (rc) {
			return rc;
		}
		...
		{
			return -EINVAL; /* unexpected use case - a NULL value or encoding problem*/
		}

		rc = base64_decode(dec_buf, sizeof(dec_buf), &olen, enc_buf, read_size);
		dec_buf[olen] = 0;
