...
static void settings_load_cb(char *name, void *val_read_cb_ctx, off_t off, void *cb_arg)
{
...
/*off - offset of value-string */
int settings_cmp(char const *val, size_t val_len, void *val_read_cb_ctx, off_t val_off)
{
...
static void settings_dup_check_cb(char *name, void *val_read_cb_ctx, off_t off, void *cb_arg)
{
	...
	if (rc || len_read == 0 || temp == '\0') {
		...
		if (cdca->val && !settings_cmp(cdca->val, cdca->val_len, val_read_cb_ctx, off)) {
			cdca->is_dup = 1;
		} else {
			cdca->is_dup = 0;
		}
